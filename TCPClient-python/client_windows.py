...
while int(choice) < 5:
    ...
    if int(choice) == 1:
        s =socket.socket()
        s.connect(('192.168.1.78', 4335))
        wasInFour = 0
        current_datetime_connection = datetime.now()
        print('Client succesfully connected')
    if int(choice) == 2: #Write a message
        # print()
        message = input('Write a client message: ')
        choice = 3
    if int(choice) == 3: #Connect+send+record+close
        ...
        with open('log_client.txt','a') as f:
            ...
            while True :
                ...
                f.write('response: ' + str(current_datetime_responce) + " - " + str(data[1:]))
                f.write('\n----------------------------------------------------------------\n\n')
                if (len(data) < 1024):
                    break
        f.close()
